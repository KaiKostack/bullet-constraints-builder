...
class bcb_props(bpy.types.PropertyGroup):
    ...
    preprocTools_dis_siz = float_(name="Minimum Size Limit",       default=2.9, min=0.0, max=1000, description="Discretization size this tool tries to reach by discretization. To enforce regularity at all times, elements can afterwards deviate in size to some extent from the target size.")
    preprocTools_dis_bol = bool_(name="Use Booleans (Accurate & Slow)", default=0, description="Enables boolean based 'halving' operations until the desired element size is reached, this helps to keep mesh details such as curved surfaces like round columns or mural reliefs. The minimum dimension value serves as limit for an element still being considered for subdivision, at least two dimension axis must be above this size. After discretization no element will be larger than this value anymore, although they can be smaller up to 50%.")
    ...
    preprocTools_fix_rng = float_(name="Boundary Range",           default=0.3, min=0, max=1000, description="Internal margin in m for the model boundary box to include also objects within a certain distance from the outer border. This value should always stay smaller than Discretization Size divided by 2 because otherwise foundation elements can overlap user elements.")
