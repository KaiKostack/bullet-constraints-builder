...
class bcb_props(bpy.types.PropertyGroup):
    ...
    preprocTools_gnd_nfq = float_(name="Frequency",                default=0.5, min=0.0, max=1000, description="Frequency of the artificial earthquake to be generated in Hz (because of the random nature of the noise function this should be taken as approximation)")
    ...
    connectionCountLimit  = int_(name="Con. Count Limit",         default=0, min=0, max=100000,    description="Maximum count of connections per object pair (0 = disabled)")
    searchDistance        = float_(name="Search Distance",        default=0.02, min=0.0, max=1000, description="Search distance to neighbor element boundary box in m")
    searchDistanceMesh    = float_(name="Mesh Search Distance",   default=0.02, min=0.0, max=1000, description="Search distance to neighbor element mesh geometry in m")
    searchDistanceFallback = bool_(name="Fallback",               default=1,                       description="In case no geometry could be detected within mesh search distance while the neighbor element's boundary box is still within range this enables a fallback using the intersection of the boundary boxes as contact area instead of the mesh surface. If disabled contact area will remain zero and no connection will be created in that case")
    ...
    detonatorMul          = float_(name="Multiplier",             default=1, min=0.0, max=1000.0,  description="Multiplier the weakening gradient strength derived from the Detonator object space will be multiplied with (not used for Advanced Detonator)")
    detonatorMax          = float_(name="Maximum",                default=1, min=0.0, max=1.0,     description="Detonator influence maximum to limit the weakening (not used for Advanced Detonator)")
    ...
    detonDeflection       = float_(name="Deflection Multiplier",  default=0, min=0.0, max=1000,    description="Detonation deflection multiplier for open/confined spaces. Larger values will increase the blast wave impact on revealed structures such as outside walls compared to protected elements inside of buildings (0 = no difference)")
    ...
    for i in range(maxMenuElementGroupItems):
        ...
        exec("elemGrp_%d_EGSidxBTX" %i +" = float_(name='Breaking Threshold Multiplier', default=presets[j][EGSidxBTX], min=0.0, max=1000000000, description='Multiplier to be applied on all breaking thresholds for constraint building. This can be useful for quickly weaken or strengthen a given element group without changing the original settings')")
        exec("elemGrp_%d_EGSidxBTI" %i +" = float_(name='Intercomponent Multiplier', default=presets[j][EGSidxBTI], min=0.0, max=1000000000, description='Multiplier to be applied on breaking thresholds that connect only different components (= multiple elements in one unit) within an element group. This can be useful to artificially weaken the strength between components')")
        ...
        exec("elemGrp_%d_EGSidxBLS" %i +" = float_(name='Shear', default=presets[j][EGSidxBLS], min=0.0, max=1000, description='Maximum distance in m through which any connected element may be moved in shearing direction without applying force or motion to the next element')")
        exec("elemGrp_%d_EGSidxBLS9" %i +" = float_(name='Shear 90°', default=presets[j][EGSidxBLS9], min=0.0, max=1000, description='Maximum distance in m through which any connected element may be moved in shearing direction, with h and w swapped (rotated by 90°), without applying force or motion to the next element')")
        exec("elemGrp_%d_EGSidxBLB" %i +" = float_(name='Bend', default=presets[j][EGSidxBLB], min=0.0, max=1000, description='Maximum angle in rad through which any connected element may be moved in bending direction without applying force or motion to the next element')")
        exec("elemGrp_%d_EGSidxBLB9" %i +" = float_(name='Bend 90°', default=presets[j][EGSidxBLB9], min=0.0, max=1000, description='Maximum angle in rad through which any connected element may be moved in bending direction, with h and w swapped (rotated by 90°), without applying force or motion to the next element')")
        ...
        exec("elemGrp_%d_EGSidxTl1D" %i +" = float_(name='1st Dist. Tol.', default=presets[j][EGSidxTl1D], min=-1.0, max=10.0, description='First deformation tolerance limit for distance change in percent for connection removal or plastic deformation (1.00 = 100 %)')")
        exec("elemGrp_%d_EGSidxTl1R" %i +" = float_(name='1st Rot. Tol.', default=presets[j][EGSidxTl1R], min=-1.0, max=pi, description='First deformation tolerance limit for angular change in radian for connection removal or plastic deformation')")
        exec("elemGrp_%d_EGSidxTl2D" %i +" = float_(name='2nd Dist. Tol.', default=presets[j][EGSidxTl2D], min=-1.0, max=10.0, description='Second deformation tolerance limit for distance change in percent for connection removal (1.00 = 100 %). The Formula Assistant might calculate this setting automatically on evaluation, it will appear greyed out then')")
