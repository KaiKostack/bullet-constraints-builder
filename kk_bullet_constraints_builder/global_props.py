...
class bcb_props(bpy.types.PropertyGroup):
    ...
    for i in range(maxMenuElementGroupItems):
        ...
        exec("elemGrp_%d_EGSidxBLB" %i +" = float_(name='Bend', default=presets[j][EGSidxBLB], min=0.0, max=1000, description='Maximum angle in rad through which any connected element may be moved in shearing direction without applying force or motion to the next element')")
        exec("elemGrp_%d_EGSidxBLB9" %i +" = float_(name='Bend 90°', default=presets[j][EGSidxBLB9], min=0.0, max=1000, description='Maximum angle in rad through which any connected element may be moved in shearing direction, with h and w swapped (rotated by 90°), without applying force or motion to the next element')")
