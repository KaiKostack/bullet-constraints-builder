...
class bcb_props(bpy.types.PropertyGroup):
    ...
    preprocTools_int_bol = bool_(name="Use Boolean Subtraction",   default=1, description="Uses boolean operations to resolve overlapping elements. Their geometries will be subtracted from each other and their collision shapes will be switched to 'Mesh'. (For accurate simulations it is strongly recommended to resolve such intersections manually and leave this option disabled.)")
