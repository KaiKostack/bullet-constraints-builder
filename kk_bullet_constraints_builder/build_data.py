...
def storeConfigDataInScene(scene):
    ...
    scene["bcb_prop_preprocTools_dis_bis"] = props.preprocTools_dis_bis
...
def getConfigDataFromScene(scene):
    ...
    if "bcb_prop_preprocTools_dis_bis" in scene.keys():
        props.preprocTools_dis_bis = scene["bcb_prop_preprocTools_dis_bis"]
...
def clearAllDataFromScene(scene, qKeepBuildData=0):
    ...
    if props.automaticMode and props.saveBackups:
        ...
        # Save file
        bpy.ops.wm.save_as_mainfile(filepath=bpy.data.filepath.split('_bake.blend')[0].split('.blend')[0] +'_bake.blend', compress=True)
